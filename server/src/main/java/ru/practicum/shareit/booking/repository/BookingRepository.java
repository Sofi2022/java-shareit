...
public interface BookingRepository extends JpaRepository<Booking, Long> {
    ...
    @Query("select b from Booking b where b.status = 'WAITING' and b.booker.id = :userId")
    List<Booking> findUserBookingsWaitingState(@Param("userId") Long userId);

    @Query("select b from Booking b where b.status = 'REJECTED' and b.booker.id = :userId")
    ...
    @Query("select b from Booking b where b.status = :state and b.item.owner.id = :ownerId")
    List<Booking> findAllBookingsByOwnerIdAndStateIgnoreCase(@Param("state") Status state, Long ownerId);

    @Query("select b from Booking b where b.booker.id = :userId and b.item.id = :itemId and b.status = :status and b.end < :time")
